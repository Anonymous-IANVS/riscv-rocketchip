...
class TLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  //val smpu = Module(new SMPUChecker(lgMaxSize))
  //smpu.io.smpu := (io.ptw.smpu: Seq[SMPU])

  //smpu.io.addr := io.req.bits.vaddr
  //smpu.io.size := io.req.bits.size
  //smpu.io.prv  := priv
  //val smpu_r = smpu.io.r
  //val smpu_w = smpu.io.w
  //val smpu_x = smpu.io.x
  //val smpu_paddr = smpu.io.paddr
  val is_smpu_working = !vm_enabled && (priv<PRV.S)

  /* Info(DD): we merge mpu_physaddr and mpu_ppn because we will always return smpu_paddr when paging is disabled */
  //val mpu_physaddr = Mux(do_refill, Cat(refill_ppn, io.req.bits.vaddr(pgIdxBits-1, 0)),
  //              Mux(vm_enabled && special_entry.nonEmpty, Cat(special_entry.map(e => e.ppn(vpn, e.getData(vpn))).getOrElse(0.U), io.req.bits.vaddr(pgIdxBits-1, 0)),
  //                  smpu_paddr)) //In the last case, we return the paddr from smpu
  val mpu_priv = Mux[UInt](Bool(usingVM) && (do_refill || io.req.bits.passthrough /* PTW */), PRV.S, Cat(io.ptw.status.debug, priv))

  //val pmp = Module(new PMPChecker(lgMaxSize))
  val pmp = Module(new PMPSMPUChecker(lgMaxSize))
  pmp.io.addr := mpu_physaddr
  pmp.io.size := io.req.bits.size
  pmp.io.pmp := (io.ptw.pmp: Seq[PMP])
  pmp.io.smpu := (io.ptw.smpu: Seq[SMPU])
  pmp.io.prv := mpu_priv

  //val legal_address = edge.manager.findSafe(mpu_physaddr).reduce(_||_)
  val legal_address = edge.manager.findSafe(pmp.io.paddr).reduce(_||_)
  def fastCheck(member: TLManagerParameters => Boolean) =
    legal_address && edge.manager.fastProperty(Mux(is_smpu_working, pmp.io.paddr, mpu_physaddr), member, (b:Boolean) => Bool(b))
    //legal_address && edge.manager.fastProperty(Mux(is_smpu_working, smpu_paddr, mpu_physaddr), member, (b:Boolean) => Bool(b))
    //legal_address && edge.manager.fastProperty(mpu_physaddr, member, (b:Boolean) => Bool(b))
  // Note(DD): making it always cachable when smpu is working
  //val cacheable = is_smpu_working || fastCheck(_.supportsAcquireB) && (instruction || !usingDataScratchpad)
  ...
  //val prot_r = is_smpu_working || (fastCheck(_.supportsGet) && !deny_access_to_debug && pmp.io.r)
  //val prot_w = is_smpu_working || (fastCheck(_.supportsPutFull) && !deny_access_to_debug && pmp.io.w)
  ...
  //val prot_x = is_smpu_working || (fastCheck(_.executable) && !deny_access_to_debug && pmp.io.x)
  ...
  /* Notes(DD): We modify the three arrary to use SMPU's permissions when satp is disabled */
  //val r_array = Cat(smpu_r, (priv_rw_ok & (entries.map(_.sr).asUInt | Mux(mxr, entries.map(_.sx).asUInt, UInt(0)))) | stage1_bypass)
  //val w_array = Cat(smpu_w, (priv_rw_ok & entries.map(_.sw).asUInt) | stage1_bypass)
  //val x_array = Cat(smpu_x, (priv_x_ok & entries.map(_.sx).asUInt) | stage1_bypass)
  val r_array = Cat(true.B, (priv_rw_ok & (entries.map(_.sr).asUInt | Mux(mxr, entries.map(_.sx).asUInt, UInt(0)))) | stage1_bypass)
  val w_array = Cat(true.B, (priv_rw_ok & entries.map(_.sw).asUInt) | stage1_bypass)
  val x_array = Cat(true.B, (priv_x_ok & entries.map(_.sx).asUInt) | stage1_bypass)
  ...
  //val pr_array = Cat(Fill(nPhysicalEntries, true.B), normal_entries.map(_.pr).asUInt) & ~(ptw_ae_array | final_ae_array)
  //val pw_array = Cat(Fill(nPhysicalEntries, true.B), normal_entries.map(_.pw).asUInt) & ~(ptw_ae_array | final_ae_array)
  //val px_array = Cat(Fill(nPhysicalEntries, true.B), normal_entries.map(_.px).asUInt) & ~(ptw_ae_array | final_ae_array)
  ...
  // DD: we only translate using smpu for U-mode.
  //io.resp.paddr := Cat(ppn, io.req.bits.vaddr(pgIdxBits-1, 0))
  io.resp.paddr := Mux( (!vm_enabled) && (priv < PRV.S), pmp.io.paddr, Cat(ppn, io.req.bits.vaddr(pgIdxBits-1, 0)))
  //io.resp.paddr := Mux( (!vm_enabled) && (priv < PRV.S), smpu_paddr, Cat(ppn, io.req.bits.vaddr(pgIdxBits-1, 0)))
  //io.resp.paddr := Mux(!vm_enabled, smpu_paddr, Cat(ppn, io.req.bits.vaddr(pgIdxBits-1, 0)))
  ...
  if (usingVM) {
    ...
    //  //Note(DD): This is the SMPU routine, in which we caculate the paddr and perm here
    //  smpu.io.addr := io.req.bits.vaddr
    //  smpu.io.size := io.req.bits.size
    //  smpu.io.prv  := priv
    //}
