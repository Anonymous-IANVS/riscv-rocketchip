...
class CSRFileIO(implicit p: Parameters) extends CoreBundle
  ...
  val smpu = Vec(nSMPUs, new SMPU).asOutput
...
class CSRFile(
  ...
  val reg_smpu = Reg(Vec(nSMPUs, new SMPUReg))
  ...
  /* SMPU CSR registers */
  val smpuCfgPerCSR = xLen / new SMPUConfig().getWidth
  def smpuCfgIndex(i: Int) = (xLen / 32) * (i / smpuCfgPerCSR)
  if (reg_smpu.nonEmpty) {
    require(reg_smpu.size <= CSR.maxSMPUs)
    val read_smpu = reg_smpu.padTo(CSR.maxSMPUs, 0.U.asTypeOf(new SMPU))
    for (i <- 0 until read_smpu.size by smpuCfgPerCSR)
      read_mapping += (CSRs.smpucfg0 + smpuCfgIndex(i)) -> read_smpu.map(_.cfg).slice(i, i + smpuCfgPerCSR).asUInt
    for ((smpu, i) <- read_smpu zipWithIndex)
      read_mapping += (CSRs.smpuoff0 + i) -> smpu.readOff
    for ((smpu, i) <- read_smpu zipWithIndex)
      read_mapping += (CSRs.smpuaddr0 + i) -> smpu.readAddr
  }
  ...
  when (csr_wen) {
    ...
    /* SMPU related logics */
    if (reg_smpu.nonEmpty) for (((smpu, next), i) <- (reg_smpu zip (reg_smpu.tail :+ reg_smpu.last)) zipWithIndex) {
      require(xLen % smpu.cfg.getWidth == 0)
      when (decoded_addr(CSRs.smpucfg0 + smpuCfgIndex(i)) && !smpu.cfgLocked) {
        val newCfg = new SMPUConfig().fromBits(wdata >> ((i * smpu.cfg.getWidth) % xLen))
        smpu.cfg := newCfg
        // disallow unreadable but writable SMPUs
        smpu.cfg.w := newCfg.w && newCfg.r
        // can't select a=NA4 with coarse-grained SMPUs
        if (pmpGranularity.log2 > SMPU.lgAlign)
          smpu.cfg.a := Cat(newCfg.a(1), newCfg.a.orR)
      }
      when (decoded_addr(CSRs.smpuaddr0 + i) && !smpu.addrLocked(next)) {
        smpu.addr := wdata
      }
      when (decoded_addr(CSRs.smpuoff0 + i) && !smpu.addrLocked(next)) {
        smpu.off := wdata
      }
    }
